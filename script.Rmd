...
max.sp_lat <- ENSEMBLE_occ.wmean |> pivot_longer(-c("longitude","latitude", "ProvId")) |> 
  filter(value>=1) |> 
  group_by(latitude) |>
  summarise(count = n_distinct(name),.groups = "keep")
# Sampling effort
effort.infer<- read_csv( "data/sampling-effort.csv",show_col_types = FALSE) |> 
  rename(n_samples_obs=total, num_sp_obs=total_occ1) |>  
  left_join(ENSEMBLE_occ.wmean.tot)  |> rename(num_sp_pred=total) |> 
...
dt1 <- sp.importance |> group_by(latitude, longitude, ProvId) |>
  ...
  drop_na()
# save data
# write_csv(dt, "data/total-range-size-rarity.csv")
# write_csv(dt1, "data/avrg-size-rarity.csv")

diver_biom<- ENSEMBLE_occ.wmean.tot |> rename(SR =total) |> # number of species per lat/lon
...
```{r}
# Filtered data : 2 species from S. Pacific
...
# save data
# write_csv(dt.fil, "data/total-range-size-rarity-FILTERED.csv")
# write_csv(dt1.fil, "data/avrg-size-rarity-FILTERED.csv")

diver_biom.fil<- ENSEMBLE_occ.wmean.tot |> rename(SR =total) |> # number of species per lat/lon
  left_join(grid|>  select("longitude","latitude","ProvId"))|>
...
#ggsave("img/LDG_SR/mapSR_UR.mean.png", width=29.7, height=14, units="cm", dpi=300)
#ggsave("img/LDG_SR/LDG_SR_UR.png", width=12, height=14, units="cm", dpi=300)
sr_patch<- SR_UR + 
...
sr_patch
#ggsave("img/mapSRvsLDG_UR.mean.png", width=27, height=12, units="cm", dpi=300)
```
...
SR_TotRar_2D <- plot.map2D(dt2D, dim=4,  xlab = "species richness",ylab = "total rarity")

dt2D <- full_join(ENSEMBLE_occ.wmean.tot[, c(1,2,4)],dt1, by = c("longitude", "latitude")) |> drop_na(sum, total)
SR_AvrgRar_2D<- plot.map2D(dt2D, dim=4,  xlab = "species richness",ylab = "average rarity")
...
### All plots combined
```{r FigureS2, fig.height=20, fig.width=15, warning=FALSE}
FigureS2<-(tot_rarity_patch/
avrg_rarity_patch/
patch2D) +
...
#ggsave("img/sampling-effort_num_sp.png", width=29.7, height=14, units="cm", dpi=300)
...
```{r FigureS1, fig.height=10, fig.width=11}
#MESO
...
```{r}
sp1<- META_zoop |>  select(name, phylum) |>  filter(phylum=="Annelida") |> mutate(col_names=paste0(name,"_wmean")) |> pull(col_names)
...
```{r FigureS5, fig.height=20, fig.width=15}
max.sp_lat <- ENSEMBLE_occ.wmean1 |> pivot_longer(-c("longitude","latitude", "ProvId")) |> 
  filter(value>=1) |> 
  group_by(latitude) |>
  summarise(count = n_distinct(name),.groups = "keep")

ldg_UR<- ENSEMBLE_occ.wmean.tot1[, c(1,2,4)] |> filter(total>0) |>
  ltg()+
  geom_line(data=max.sp_lat , aes(x=latitude,y= count),color="grey70")+
  labs(x="Latitude (cell midpoint)",color="Species Richness",y="Species Richness")
SR_UR<- plot.map(ENSEMBLE_occ.wmean.tot1[,c("longitude","latitude","total")] )+labs(color="Species Richness")
...
sp.importance <- ENSEMBLE_occ.wmean1 |> 
  group_by(longitude, latitude, ProvId) |> pivot_longer(-c("longitude","latitude","ProvId")) |> drop_na(value) |> filter(value>0)|>
  group_by(name) |> mutate(N=n()) |> ungroup() |> #mutate(p=N/nrow(grid))
  mutate(R=value/N)


dt<- sp.importance |> group_by(latitude, longitude, ProvId) |> summarize(sum = sum(R, na.rm=TRUE), .groups="keep") |>  ungroup() |> right_join(grid, by = c("latitude", "longitude","ProvId")) |> drop_na(ProvId) |> select(longitude, latitude, sum)|> mutate(sum=scales::rescale(sum))|> drop_na()
dt1<- sp.importance |> group_by(latitude, longitude, ProvId) |> summarize(n=n(),sum = sum(R, na.rm=TRUE)/n, .groups="keep") |>  ungroup() |> right_join(grid, by = c("latitude", "longitude","ProvId")) |> drop_na(ProvId) |> select(longitude, latitude, sum)|> mutate(sum=scales::rescale(sum))|> drop_na()
...
avrg_rarity_patch<-avrg_rarity+ theme(legend.position="none") + ldg_avrg_rarity+ 
  plot_layout(widths = c(3.0, 1))+
  #plot_annotation(tag_levels = 'A')+ 
  plot_layout(guides = 'collect')
...
dt2D <- full_join(ENSEMBLE_occ.wmean.tot1[, c(1,2,4)],dt1, by = c("longitude", "latitude")) |> drop_na(sum, total)
# create classes

# before projecting, transform NE_places to data frame to use it inside ggplot()
NE_places.df <- bi_class(dt2D, x = total, y = sum, style = "equal", dim = dim)
names(NE_places.df) <- c("lon", "lat","PROB","PROB1","bi_class")
NE_places.dt.prj <- data.table(NE_places.df,NE_places.df)
# Create an sf object from the data frame
sf_object <- st_as_sf(NE_places.df, coords = c("lon", "lat"), crs = 4326)
...
## Arthropoda -quantile
```{r}
sp1<- META_zoop |>  select(name, phylum) |>  filter(phylum=="Arthropoda") |> mutate(col_names=paste0(name,"_wmean")) |> pull(col_names)

ENSEMBLE_occ.wmean1<-ENSEMBLE_occ.wmean |> select(longitude,latitude,ProvId,any_of(sp1))
ENSEMBLE_occ.wmean.tot1 <- sums(ENSEMBLE_occ.wmean1)|> drop_na(ProvId)

```

```{r FigureS6, fig.height=20, fig.width=15}
max.sp_lat <- ENSEMBLE_occ.wmean1 |> pivot_longer(-c("longitude","latitude", "ProvId")) |> 
  filter(value>=1) |> 
  group_by(latitude) |>
  summarise(count = n_distinct(name),.groups = "keep")
...
```{r}
sp1<- META_zoop |>  select(name, phylum) |>  filter(phylum=="Chaetognatha") |> mutate(col_names=paste0(name,"_wmean")) |> pull(col_names)
...
```{r FigureS7, fig.height=20, fig.width=15}
max.sp_lat <- ENSEMBLE_occ.wmean1 |> pivot_longer(-c("longitude","latitude", "ProvId")) |> 
  filter(value>=1) |> 
  group_by(latitude) |>
  summarise(count = n_distinct(name),.groups = "keep")
...
```{r}
sp1<- META_zoop |>  select(name, phylum) |>  filter(phylum=="Chordata") |> mutate(col_names=paste0(name,"_wmean")) |> pull(col_names)
...
```{r FigureS8, fig.height=20, fig.width=15}
max.sp_lat <- ENSEMBLE_occ.wmean1 |> pivot_longer(-c("longitude","latitude", "ProvId")) |> 
  filter(value>=1) |> 
  group_by(latitude) |>
  summarise(count = n_distinct(name),.groups = "keep")
...
patch2D) +
  ...
  plot_annotation(tag_levels = 'A')& 
  theme(plot.tag = element_text("Times New Roman",face = "bold", size = 12,hjust = -3, vjust = 0))
ggsave("img/FigureS4.png", width=30, height=42, units="cm", dpi=300)
```
## Cnidaria-quantile
```{r}
sp1<- META_zoop |>  select(name, phylum) |>  filter(phylum=="Cnidaria") |> mutate(col_names=paste0(name,"_wmean")) |> pull(col_names)
...
```{r FigureS9, fig.height=20, fig.width=15}
max.sp_lat <- ENSEMBLE_occ.wmean1 |> pivot_longer(-c("longitude","latitude", "ProvId")) |> 
  filter(value>=1) |> 
  group_by(latitude) |>
  summarise(count = n_distinct(name),.groups = "keep")
...
patch2D) +
  ...
  plot_annotation(tag_levels = 'A')& 
  theme(plot.tag = element_text("Times New Roman",face = "bold", size = 12,hjust = -3, vjust = 0))
ggsave("img/FigureS5.png", width=30, height=42, units="cm", dpi=300)
```
## Ctenophora- equal (dim=3)
```{r}
sp1<- META_zoop |>  select(name, phylum) |>  filter(phylum=="Ctenophora") |> mutate(col_names=paste0(name,"_wmean")) |> pull(col_names)
...
```{r FigureS10, fig.height=20, fig.width=15}
max.sp_lat <- ENSEMBLE_occ.wmean1 |> pivot_longer(-c("longitude","latitude", "ProvId")) |> 
  filter(value>=1) |> 
  group_by(latitude) |>
  summarise(count = n_distinct(name),.groups = "keep")
...
sp.importance <- ENSEMBLE_occ.wmean1 |> 
  group_by(longitude, latitude, ProvId) |> pivot_longer(-c("longitude","latitude","ProvId")) |> drop_na(value) |> filter(value>0)|>
  group_by(name) |> mutate(N=n()) |> ungroup() |> 
  mutate(R=value/N)
...
patch2D) +
  ...
  plot_annotation(tag_levels = 'A')& 
  theme(plot.tag = element_text("Times New Roman",face = "bold", size = 12,hjust = -3, vjust = 0))
ggsave("img/FigureS6.png", width=30, height=42, units="cm", dpi=300)
```

## Mollusca-quantile
```{r}
sp1<- META_zoop |>  select(name, phylum) |>  filter(phylum=="Mollusca") |> mutate(col_names=paste0(name,"_wmean")) |> pull(col_names)
...
```{r FigureS11, fig.height=20, fig.width=15}
max.sp_lat <- ENSEMBLE_occ.wmean1 |> pivot_longer(-c("longitude","latitude", "ProvId")) |> 
  filter(value>=1) |> 
  group_by(latitude) |>
  summarise(count = n_distinct(name),.groups = "keep")
...
patch2D) +
  ...
  plot_annotation(tag_levels = 'A')& 
  theme(plot.tag = element_text("Times New Roman",face = "bold", size = 12,hjust = -3, vjust = 0))
ggsave("img/FigureS7.png", width=30, height=42, units="cm", dpi=300)
```
